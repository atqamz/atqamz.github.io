...
- `apps/short`: static shortlink site generated from `data/links.json`, which remains the single source for shortlinks.
- `apps/meet`: static Cloudflare Pages redirect for `meet.atqamz.com`; keep it independent from `data/links.json`.
...
Cloudflare Pages Git integration builds and deploys all four applications from GitHub. Pulumi Deployments is the only path that applies infrastructure changes. GitHub Actions is validation-only and must not hold production deployment credentials.
